...
namespace chkam05.ZtmDataDownloader.Serialization
{
    public class LinesSerializer : BaseSerializer<LinesResponseModel>
    {
        ...
        private Dictionary<TransportType, List<LineItem>> ReadData(Dictionary<TransportType, XElement> groupedDataNodes)
        {
            var result = new Dictionary<TransportType, List<LineItem>>();
        ...
        private List<LineItem> ReadLinesData(IEnumerable<XElement> linesDataNodes, TransportType transportType)
        {
        ...
        private LineItem ReadSingleLineData(XElement lineDataNode, TransportType transportType)
        {
            return new LineItem()
            {
        ...
        private List<string> ReadLineAttributes(XElement lineDataNode)
        {
            if (lineDataNode.Attributes().Any(a => a.Name == "class"))
            {
                ...
                if (!string.IsNullOrEmpty(attributes?.Value))
                    return attributes.Value.Split(' ').ToList();
            }
